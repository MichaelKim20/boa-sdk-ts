...
describe ('Test for UTXOManager', () =>
{
    ...
    it ('Test that no data with the same UTXO is added', () =>
    {
        let sum1 = manager.getSum();
        manager.add(utxos);
        let sum2 = manager.getSum();
    ...
    {
        // (unlock_height - 1 <= 1)
        let sum = manager.getSum(BigInt(1));
        assert.deepStrictEqual(sum, [BigInt(10), BigInt(10), BigInt(80)])

        sum = manager.getSum();
        assert.deepStrictEqual(sum, [BigInt(90), BigInt(10), BigInt(0)])
    });

    it ('Obtain a UTXO to use - if there is enough money', () =>
    {
        let res = manager.getUTXO(BigInt(15), BigInt(5));
        assert.deepStrictEqual(res, [utxos[1], utxos[2]]);
    });

    it ('Obtain a UTXO to use - if there is not enough money', () =>
    {
        let sum = manager.getSum(BigInt(10));
        assert.deepStrictEqual(sum, [BigInt(70), BigInt(10), BigInt(0)])

        let res = manager.getUTXO(sum[0] + BigInt(1), BigInt(10));
