...
export class TxBuilder
{
    ...
    public sign (type: TxType = TxType.Payment) : Transaction
    {
        ...
        for (let elem of this.inputs)
            inputs.push(new TxInput(elem.utxo, new Signature(Buffer.alloc(Signature.Width))));

        for (let elem of this.outputs)
            outputs.push(new TxOutput(elem.value, elem.address));

        let tx = new Transaction(type, inputs, outputs,
            (
                (this.payload !== undefined)
                    ? this.payload
        ...
        for (let idx = 0; idx < this.inputs.length; idx++)
            tx.inputs[idx].signature = this.inputs[idx].key.sign(tx_hash.data);
