...
export class VarInt
{
    ...
    public static fromNumber (value: number, buffer: SmartBuffer)
    {
        ...
        else if (value <= 0xFFFFFFFF)
        {
            ...
            throw new Error("The number is too large to be processed.");
    }
    ...
    public static toNumber (buffer: SmartBuffer): number
    {
        ...
        if (remaining < 1)
            throw new Error(`Requested 1 bytes but only ${remaining} bytes available`);
        ...
        else if (value == number_type[1])
        {
            ...
            if (remaining < 2)
                throw new Error(`Requested 2 bytes but only ${remaining} bytes available`);
        ...
        else if (value == number_type[2])
        {
            remaining = buffer.remaining();
            if (remaining < 4)
                throw new Error(`Requested 4 bytes but only ${remaining} bytes available`);
    ...
    public static toJSBI (buffer: SmartBuffer): JSBI
    {
        ...
        else
        {
            remaining = buffer.remaining();
            if (remaining < 8)
                throw new Error(`Requested 8 bytes but only ${remaining} bytes available`);
